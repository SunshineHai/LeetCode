...
# 形参含义：(训练集样本编号, 测试集样本编号, 全部样本编号共405条, 训练集的辛烷值预测值, 测试集的辛烷值的预测值, 辛烷值含量的原始值)
...
def show_error(row_name: list, RMSE_data: list, MAE_data, run_time_data: list, path):
    #     plt.figure(figsize=(10, 10))
    plt.plot(row_name, RMSE_data, 'or-', MAE_data, '^g-', run_time_data, '*b-')
    plt.legend(['RMSE', 'MAE', 'Run_Time'], loc='upper right')
...
# 3. 特征工程(归一化)
data, _, _ = normalization(all_data[:, 1:-1])  # 进行归一化

# 4.机器学习

models = [LinearRegression(normalize=True), MLPRegressor(alpha=0.01), DecisionTreeRegressor(), SVR(),
          GradientBoostingRegressor(),
          lgb.LGBMRegressor(objective='regression', num_leaves=31, learning_rate=0.05, n_estimators=20)]
models_str = ['LR', 'MLPRegressor', 'DecisionTree', 'SVM', 'GBDT', 'lightGBM']
precision_data = []
recall_data = []
f1_data = []
run_time_data = []
...
path = r'画图数据\show_error.tif'
print(precision_data, '\n', recall_data, '\n', f1, '\n', AUC, '\n', run_time_data)
show_error(precision_data, recall_data, f1, AUC, run_time_data, path)
